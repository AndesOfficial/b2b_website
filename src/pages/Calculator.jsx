              ...
              {[[100,0],[83.33,16.67],[70,30],[50,50],[0,100]].map(([cold,hot]) => (
                <button key={cold} onClick={() => { set("heaterColdPct", cold); set("heaterHotPct", hot); }}
                  className={`text-[10px] font-semibold px-2 py-1 rounded-lg border transition
                    ${Math.abs(a.heaterColdPct-cold)<0.1?"bg-slate-700 text-white border-slate-700":"bg-white text-slate-500 border-slate-200 hover:border-slate-300"}`}>
                  {cold}:{hot}
                </button>
              ))}
            </div>
  ...
  return (
    <div className="flex min-h-screen bg-slate-50" style={{ fontFamily: "DM Sans, sans-serif" }}>
      ...
      <main className={`flex min-h-screen flex-1 flex-col transition-all duration-300 ${isSidebarCollapsed?"lg:ml-[80px]":"lg:ml-[220px]"} ml-0`}>
        ...
        <div className="bg-amber-50 border-b border-amber-200 px-6 py-2.5 flex items-center justify-between text-amber-900 text-xs font-bold z-10 relative shadow-sm">
          <div className="flex items-center gap-2">
            <FiInfo className="text-amber-600 shrink-0" size={15} />
            <span>PROJECTION WATERMARK: All figures generated by this calculator are simulated financial projections and hypothetical estimates for planning purposes only. They do not represent historical accounting or actual financial records of Andes.</span>
          </div>
          <span className="bg-amber-200/80 text-amber-900 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider shrink-0 ml-4">SIMULATION ONLY</span>
        </div>

        <div className="flex flex-col lg:flex-row flex-1 min-h-0 relative">
          <div className="pointer-events-none absolute inset-0 z-0 flex items-center justify-center overflow-hidden opacity-[0.03] select-none">
            <span className="text-[120px] font-black uppercase tracking-widest text-slate-900 -rotate-12 whitespace-nowrap">
              PROJECTION ONLY · NOT ACTUAL DATA
            </span>
          </div>
          <div className="w-full lg:w-[310px] xl:w-[350px] flex-shrink-0 bg-white border-b lg:border-b-0 lg:border-r border-slate-200 overflow-y-auto z-10 relative">
            <div className="p-4">
